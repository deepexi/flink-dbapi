...
class FlinkOperation:
    ...
    def _execute_statement_with_job_name(self, sql: str, job_name: str = None) -> None:
        ...
        """
        if job_name:
            self.last_flink_job_name = job_name
            sql_list = [FLINK_SQL_SET_JOB_NAME.format(job_name), sql]
            self.last_operation = SqlGatewayOperation.submit_sql_and_wait_submit_finished(
            ...
            if self.is_flink_job_mode_streaming():
    ...
    def is_flink_job_mode_streaming(self):
        if self.last_flink_job_name is None:
            raise Exception("last_operation_job_name is None")
            # return False
        job: FlinkJob = self.flink_rest_client.get_latest_start_job_by_name(
            self.last_flink_job_name
        )
        if job is None:
            # return False
            raise Exception(
                f"No such job name={self.last_flink_job_name}, maybe finished too long"
            )
    ...
    def _wait_job_complete(self):
        if self.last_flink_job_name:
            self.flink_rest_client.wait_job_complete(self.last_flink_job_name)

    def _wait_job_running(self):
        if self.last_flink_job_name:
            self.flink_rest_client.wait_job_running(self.last_flink_job_name)

    def cancel_job(self):
        if self.last_flink_job_name:
            self.flink_rest_client.cancel_job_by_name_if_possible(self.last_flink_job_name)
